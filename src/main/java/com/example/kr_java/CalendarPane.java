...
public class CalendarPane extends BorderPane {
    ...
    private void showDayNames() { //показва имената на дните
        DateFormatSymbols dfs = new DateFormatSymbols(locale); //клас за форматиране на дата и час
        String dayNames[] = dfs.getWeekdays();
        //getWeekdays - получаване на масив от имена на дни
    ...
    private void showHeader() {
        SimpleDateFormat sdf = new SimpleDateFormat("MMMM yyyy", locale);
                //персонализираме формата на датата

        String header = sdf.format(calendar.getTime());
        lblHeader.setText(header);
    }
//update names of days based on locale
